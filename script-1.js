...
function createMoshStamp(yPos) {
  ...
  // Max 2 stamps at a time — remove oldest if over limit
  ...
  stamp.animate([
    { opacity: 0.6, transform: 'translateY(0px) scale(1)' },
    { opacity: 0,   transform: 'translateY(15px) scale(1.005)' }
  ], { duration: 250, easing: 'ease-out' }).onfinish = () => stamp.remove();
}
  stamp.animate([
    { opacity: 0.7, transform: 'translateY(0px) scale(1)' },
    { opacity: 0,   transform: 'translateY(25px) scale(1.01)' }
  ], { duration: 400, easing: 'cubic-bezier(0.25, 1, 0.5, 1)' }).onfinish = () => stamp.remove();
}
