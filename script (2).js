...
// Reusable SVG clipPath for mosh stamps
const moshSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
moshSvg.style.cssText = 'position:absolute;width:0;height:0;';
const moshClipPath = document.createElementNS('http://www.w3.org/2000/svg', 'clipPath');
moshClipPath.id = 'mosh-clip';
moshClipPath.setAttribute('clipPathUnits', 'userSpaceOnUse');
moshSvg.appendChild(moshClipPath);
document.body.appendChild(moshSvg);

function createMoshStamp(yPos) {
  if (isAlbumOpen) return;

  // READ — batch all rects before touching DOM
  const targets = document.querySelectorAll('.mosh-target');
  ...
  targets.forEach(el => {
    ...
    if (r.bottom > 0 && r.top < vh) visibleRects.push(r);
  });

  if (!visibleRects.length) return;

  // WRITE — build clipPath rects then stamp once
  moshClipPath.innerHTML = visibleRects.map(r =>
    `<rect x="${r.left}" y="${r.top}" width="${r.width}" height="${r.height}"/>`
  ).join('');

  const stamp = document.createElement('div');
  stamp.className = 'brush-stamp';
  stamp.style.top = `-${scrollY}px`;
  stamp.style.clipPath = 'url(#mosh-clip)';
