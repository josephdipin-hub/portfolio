...
function createMoshStamp(yPos) { 
  ...
  // Collect hero text + all keyword spans
  const targets = [
    document.querySelector('.hero-text'),
    ...document.querySelectorAll('[data-mosh]')
  ].filter(el => el !== null);

  targets.forEach(target => {
    const rect = target.getBoundingClientRect();

    // Only stamp if element is currently visible in viewport
    ...
    stamp.appendChild(target.cloneNode(true)); 
