...
/* ═══════════════════════════════════════════════════════
   GALLERY CAMERA — TAP TO OPEN
   Resting state shows the closed camera (GALLERY branding on the
   hood) with a "TAP TO OPEN" hint. Tapping fires a short haptic
   pulse and plays the hood-opening clip once; when it ends, the
   frame freezes open on the blank ground glass and the two gallery
   headlines fade in on top of it. Tapping either headline opens the
   existing fashion/product galleries exactly as before.
════════════════════════════════════════════════════════ */
(function () {
  const stage    = document.getElementById('gallery-camera-stage');
  const poster   = document.getElementById('gallery-camera-poster');
  const video    = document.getElementById('gallery-camera-video');
  const hitArea  = document.getElementById('gallery-camera-hit');
  const tapHint  = document.getElementById('gallery-tap-hint');
  const fashionBtn = document.getElementById('open-fashion-gallery');
  const productBtn = document.getElementById('open-product-gallery');
  if (!stage || !poster || !video || !hitArea) return;
  ...
  function openCamera() {
    ...
    // The source clip holds on the closed camera for its first ~1.3s
    // before the hood visibly starts opening — skip straight to that
    // moment so there's no dead wait after the tap.
    const startAt = () => {
      try { video.currentTime = 1.3; } catch (e) {}
