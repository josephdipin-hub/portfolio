...
function initDarkroom() {
  ...
  drScene.fog = new THREE.FogExp2(0x0a0002, 0.06);
  ...
  /* ── Lighting: darkroom safelight ── */
  const ambient = new THREE.AmbientLight(0x220008, 3.0);
  drScene.add(ambient);

  // Red safelight overhead — warm, not blinding
  const safelight = new THREE.PointLight(0xff2200, 1.2, 25);
  safelight.position.set(0, 4, 2);
  safelight.castShadow = false;
  drScene.add(safelight);

  // Chemical glow from trays
  const trayGlow = new THREE.PointLight(0xff1100, 0.4, 12);
  trayGlow.position.set(-1, -1.5, 3);
  drScene.add(trayGlow);

  // Soft front fill — makes photos readable
  const frontFill = new THREE.PointLight(0xff1800, 0.6, 18);
  frontFill.position.set(0, 1, 6);
  drScene.add(frontFill);

  // Enlarger — warm white over right table
  const enlargerLight = new THREE.SpotLight(0xfff5e0, 2.5, 8, Math.PI / 7, 0.5);
  enlargerLight.position.set(2.5, 3, 1);
  enlargerLight.target.position.set(2.5, -1, 0);
  enlargerLight.castShadow = false;
...
function makeImagePlane(src, w, h) {
  ...
  // Dark placeholder — visible under safelight
...
function applyTexture(mesh, tex, w, h) {
  ...
  // MeshBasicMaterial shows full image — safelight tint via color
  mesh.material = new THREE.MeshBasicMaterial({
    map: tex,
    transparent: true,
    opacity: 0.0,
    color: 0xffcccc,  // very subtle warm tint — safelight feel without killing the image
  });
  ...
  function fadeIn() {
    ...
    else mesh.material.opacity = 0.92;
  }
  fadeIn();
}
...
/* ── Zone navigation ────────────────────────────────── */
const ZONE_CAMERAS = {
  contact:  { x: 0,    y: 0.5,  z: 5 },   // back wall — sneakers
  tray:     { x: -2.5, y: -0.5, z: 4 },   // left table — woven
  line:     { x: 0,    y: 1.8,  z: 4 },   // overhead wire — ties
  enlarger: { x: 2.5,  y: -0.5, z: 4 },   // right table — eyewear
  wall:     { x: -4,   y: 0,    z: 3 },   // side wall — cookware
};
const ZONE_ORDER = ['contact', 'tray', 'line', 'enlarger', 'wall'];
let currentZoneIdx = 0;
let drTargetPos = { x: 0, y: 0.5, z: 5 };

function navigateToZone(idx) {
  idx = Math.max(0, Math.min(ZONE_ORDER.length - 1, idx));
  currentZoneIdx = idx;
  const zone = ZONE_ORDER[idx];
  drTargetPos = { ...ZONE_CAMERAS[zone] };

  // Update hint label
  const zoneLabel = PRODUCT_ZONES[zone]?.label || '';
  drHint.textContent = zoneLabel;
  drHint.style.opacity = '1';
  setTimeout(() => { drHint.style.opacity = '0'; }, 2000);
}

// Touch swipe for zone navigation
let drTouchStartX = 0;
let drTouchStartY = 0;

function onDrTouchStart(e) {
  drTouchStartX = e.touches[0].clientX;
  drTouchStartY = e.touches[0].clientY;
}
...
function onDrTouchEnd(e) {
  ...
  if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 40) {
    // Horizontal swipe — navigate zones
    navigateToZone(currentZoneIdx + (dx < 0 ? 1 : -1));
  }
}

// Wheel for zone navigation on desktop
function onDrWheel(e) {
  if (!isDarkroomOpen) return;
  e.preventDefault();
  navigateToZone(currentZoneIdx + (e.deltaY > 0 ? 1 : -1));
}
...
function darkroomRenderLoop() {
  ...
  // Smooth camera to zone target + parallax offset
  const targetX = drTargetPos.x + drCamX * 0.8;
  const targetY = drTargetPos.y + drCamY * 0.4;
  const targetZ = drTargetPos.z;

  drCamera.position.x += (targetX - drCamera.position.x) * 0.04;
  drCamera.position.y += (targetY - drCamera.position.y) * 0.04;
  drCamera.position.z += (targetZ - drCamera.position.z) * 0.04;
  ...
  drCamera.lookAt(
    drTargetPos.x + drCamX * 0.2,
    drTargetPos.y + drCamY * 0.1,
    0
  );
...
function toggleProductGallery(open) {
  ...
  if (open) {
    ...
    // Show zone hint
    drHint.textContent = PRODUCT_ZONES['contact'].label;
    drHint.style.opacity = '1';
