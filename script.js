...
const VignetteShader = {
  ...
  fragmentShader: `
    ...
    void main(){
      ...
      vec3 hazeColor = vec3(0.04, 0.01, 0.00);
...
function initDarkroom() {
  ...
  drScene.fog = new THREE.FogExp2(0x0a0301, 0.085);
...
function buildRoom() {
  ...
  const wallMat  = new THREE.MeshStandardMaterial({ color:0x050200, roughness:0.97, metalness:0.0 });
  const floorMat = new THREE.MeshStandardMaterial({ color:0x020100, roughness:0.99, metalness:0.0 });
  ...
  drScene.add(new THREE.AmbientLight(0x1a0601, 1.2));

  // PRIMARY SAFELIGHT — large warm orange-red, generous range
  const safe = new THREE.PointLight(0xff3800, 28.0, 22, 2.2);
  ...
  const safe2 = new THREE.PointLight(0xff1500, 5.0, 12, 2.5);
  ...
  const backWash = new THREE.PointLight(0xff4808, 2.0, 8, 2.5);
  ...
  const rimLeft = new THREE.PointLight(0xff1800, 1.2, 8, 2.5);
...
function placeSceneObjects() {
  ...
  cloneModel('polaroid', 0.28, [-6.3, 1.82, -4.1], [0, Math.PI / 4, 0]);
...
function buildDryingLine() {
  ...
  const WIRE_Y = 2.5; // wire is at y=2.5

  imgs.forEach((img, i) => {
    const mesh = buildPhotoPlane(img.src, 0.7, 1.05);
    const photoX = sX + i * spacing;
    const photoZ = -0.3 + rots[i % rots.length] * 0.5;
    // Photo top edge sits just below wire — photo hangs from wire
    const photoY = WIRE_Y - 0.55; // half photo height below clip attach point
    mesh.position.set(photoX, photoY, photoZ);
    mesh.rotation.z = rots[i % rots.length];
    mesh.userData = { zone: 'line', imgIndex: i, label: PRODUCT_ZONES.line.label };
    drPhotoMeshes.push(mesh);
    drScene.add(mesh);

    // 2 clips per photo — left and right top corners
    [-0.18, 0.18].forEach(offsetX => {
      const clip = drModelCache['clip'] ? drModelCache['clip'].clone(true) : null;
      if (clip) {
        // Scale clip to be clearly visible — about 8cm tall in scene units
        const clipBox = new THREE.Box3().setFromObject(clip);
        const clipSize = new THREE.Vector3();
        clipBox.getSize(clipSize);
        const clipMaxDim = Math.max(clipSize.x, clipSize.y, clipSize.z);
        const targetClipHeight = 0.09;
        clip.scale.setScalar(clipMaxDim > 0 ? targetClipHeight / clipMaxDim : 0.055);

        // Snap clip to wire — clip top sits on wire
        clip.position.set(
          photoX + offsetX,
          WIRE_Y - 0.04, // clip body just below wire centre
          photoZ
        );
        clip.rotation.z = rots[i % rots.length];
        clip.rotation.x = 0;
        clip.traverse(n => { if (n.isMesh) { n.castShadow = true; n.receiveShadow = true; } });
        drScene.add(clip);
      }
    });

    // Thin thread from wire down to photo top
    [-0.18, 0.18].forEach(offsetX => {
      drScene.add(new THREE.Line(
        new THREE.BufferGeometry().setFromPoints([
          new THREE.Vector3(photoX + offsetX, WIRE_Y, photoZ),
          new THREE.Vector3(photoX + offsetX, photoY + 0.52, photoZ)
        ]),
        new THREE.LineBasicMaterial({ color: 0x222222 })
      ));
    });
