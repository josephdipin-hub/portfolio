...
function createMoshStamp(yPos) {
  ...
  // Find the highest point where a no-mosh section is visible on screen.
  // We want the smallest positive top value (first no-mosh edge entering from bottom),
  // or if any no-mosh section already occupies top of screen, bail entirely.
  const noMoshEls = document.querySelectorAll('[data-no-mosh]');
  let safeHeight = vh; // how many px from top of viewport are safe to stamp

  noMoshEls.forEach(el => {
    const rect = el.getBoundingClientRect();
    // Section is at least partially on screen
    if (rect.top < vh && rect.bottom > 0) {
      // Its top edge is where the mosh must stop
      const edge = Math.max(0, rect.top);
      if (edge < safeHeight) safeHeight = edge;
    }
  });

  // Nothing safe to stamp — entire viewport is covered by no-mosh sections
  ...
  // Clip from bottom so stamp only covers safe area
  if (safeHeight < vh) {
...
function applyMood() {
  ...
  // 1. Hero overlay — CSS filter on the clip overlay element
  const heroOverlay = document.getElementById('hero-overlay');
  if (heroOverlay) heroOverlay.style.filter = cfg.heroFilter;

  // 2. Gallery trigger images
  ...
  // 3. Subtle page tint via a fixed overlay div
  let tintEl = document.getElementById('mood-tint');
  if (!tintEl) {
  ...
  tintEl.style.background = cfg.tint;

  // 4. Label in hero
  const moodLabel = document.getElementById('mood-label');
  if (moodLabel) moodLabel.textContent = cfg.label;

  // 5. Store on body for CSS hooks if needed
