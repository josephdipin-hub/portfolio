...
var PG_NOISE_FRAG = `
  ...
  void main() {
    ...
    /* Scanlines */
    float scan = sin(vUv.y * uRes.y * 0.8 + t * 12.0) * 0.018;

    /* Reddish-purple base palette — davidlangarica mood */
    vec3 col1 = vec3(0.28, 0.0,  0.18); /* deep crimson-purple */
    vec3 col2 = vec3(0.08, 0.0,  0.22); /* dark indigo */
    vec3 col3 = vec3(0.55, 0.05, 0.12); /* vivid red accent */
    vec3 col  = mix(col1, col2, n);
    col       = mix(col, col3, pow(n1, 3.0) * 0.5);
    col      += scan * vec3(0.4, 0.0, 0.2);

    /* Vignette */
    vec2  vig = vUv * (1.0 - vUv.yx);
    float v   = pow(vig.x * vig.y * 18.0, 0.5);
    col      *= mix(0.0, 1.0, v);

    /* Grain */
    float grain = hash(vUv * uRes + t * 137.0) * 0.04;
    col += grain * vec3(0.6, 0.1, 0.4);
...
function initEnlargerBg() {
  ...
  pgRenderer = new THREE.WebGLRenderer({ canvas: canvas, antialias: true, alpha: false });
  ...
  loader.load('models/durst_enlarger_darkroom_asset.glb', function(gltf) {
    ...
    /* ── METAL FINISH — chrome-like ── */
    pgEnlargerModel.traverse(function(n) {
      if (!n.isMesh) return;
      /* Replace every material with a polished metal */
      var oldMat = n.material;
      var basecol = new THREE.Color(0x1a0010); /* very dark reddish tint */
      /* Tint based on original colour if present */
      if (oldMat && oldMat.color) {
        var lum = oldMat.color.r * 0.299 + oldMat.color.g * 0.587 + oldMat.color.b * 0.114;
        basecol.setRGB(
          0.08 + lum * 0.15,
          0.0  + lum * 0.05,
          0.12 + lum * 0.18
    ...
    /* Scale + center — pulled back so full model fits */
    ...
    /* Scale so the tallest dimension = 3.5 units — camera at z=7 sees it all */
