...
function initEnlargerBg() {
  ...
  pgRenderer.toneMappingExposure = 0.85;
  pgRenderer.outputEncoding = THREE.sRGBEncoding;

  pgScene  = new THREE.Scene();
  pgCamera = new THREE.PerspectiveCamera(40, window.innerWidth / window.innerHeight, 0.1, 100);
  pgCamera.position.set(0, 0.2, 3.5);
  pgCamera.lookAt(0, 0, 0);

  /* Warm safelight rig */
  pgScene.add(new THREE.AmbientLight(0x1a0600, 3.5));
  var key = new THREE.PointLight(0xff4400, 14, 20, 1.8);
  key.position.set(2, 4, 3);
  pgScene.add(key);
  var fill = new THREE.PointLight(0xff2200, 5, 12, 2);
  fill.position.set(-3, 1, 1);
  pgScene.add(fill);
  var top = new THREE.DirectionalLight(0xffc070, 1.4);
  ...
  loader.load('models/durst_enlarger_darkroom_asset.glb', function(gltf) {
    ...
    pgEnlargerModel.traverse(function(n) {
      if (n.isMesh && n.material) {
        ...
        n.material.metalness = Math.min((n.material.metalness || 0) * 0.5, 0.3);
      }
    });
    /* Fit to view */
    ...
    var sc  = 2.6 / Math.max(sz.x, sz.y, sz.z);
...
function enlargerLoop() {
  ...
  if (pgEnlargerModel) {
    pgEnlargerModel.rotation.y = Math.sin(pgTime * 0.15) * 0.14;
  }
  if (pgCamera) {
    pgCamera.position.x = Math.sin(pgTime * 0.10) * 0.12;
    pgCamera.lookAt(0, 0, 0);
  }
