...
/* ===== DYPIN CINEMATIC DARKROOM PATCH ===== */
(function(){
  const label = document.getElementById('darkroom-label');
  const hint = document.getElementById('darkroom-hint');
  ...
  if(label){
    const zones = [
      'ENLARGER_ARCHIVE',
      'LIGHT_TEST_CHAMBER',
      'FLOATING_CONTACT_SHEETS',
      'PRODUCT_MEMORY_FIELD',
      'CHEMICAL_REFLECTION_ZONE'
    ];
    ...
    setInterval(()=>{
      ...
      setTimeout(()=>{
        label.textContent = zones[i % zones.length];
        label.style.opacity = '1';
        i++;
      },400);
    },4200);
  ...
  if(hint){
    hint.textContent = 'SCROLL_TO_DRIFT';
  }

  let drift = 0;
  window.addEventListener('scroll', ()=>{
    drift += 0.12;
    document.documentElement.style.setProperty('--mosh-drift', drift.toFixed(2));

    const canvas = document.getElementById('darkroom-canvas');
    if(canvas){
      const blur = Math.min(window.scrollY * 0.0008, 1.8);
      canvas.style.filter = `contrast(1.08) brightness(0.92) blur(${blur}px)`;
    }
  }, { passive:true });
})();
