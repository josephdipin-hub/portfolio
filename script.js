...
function initDarkroom() {
  ...
  const ambient = new THREE.AmbientLight(0x0a0005, 1.0);
  drScene.add(ambient);

  // Red safelight — the only real light source
  const safelight = new THREE.PointLight(0x8B0000, 2.5, 18);
  safelight.position.set(0, 4, 2);
  safelight.castShadow = true;
  drScene.add(safelight);

  // Dim fill from below (chemical glow from trays)
  const trayGlow = new THREE.PointLight(0x1a0800, 0.8, 8);
  trayGlow.position.set(-1, -1.5, 3);
  drScene.add(trayGlow);

  // Enlarger cone of light — white, tight, over the right table
  const enlargerLight = new THREE.SpotLight(0xfff5e0, 1.2, 10, Math.PI / 8, 0.4);
