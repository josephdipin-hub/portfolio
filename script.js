...
function initEnlargerBg() {
  ...
  function updatePct() {
    ...
    if (pctEl) pctEl.textContent = total < 100 ? `${stage} // ${total}%` : stage;
    if (total >= 100 && loaderEl) { setTimeout(() => loaderEl.classList.add('hidden'), 800); }
  }
  const loader = new THREE.GLTFLoader();
  loader.load('models/durst_enlarger_darkroom_asset.glb', (gltf) => {
      enlargerPct = 100; updatePct();
      ...
      pgEnlargerModel.traverse((n) => { if (n.isMesh && n.material) n.material.envMap = cubeRT.texture; });
      canvas.classList.add('visible');
      if (productScroll) productScroll.classList.add('ready');
      const loader2 = new THREE.GLTFLoader();
      loader2.load('models/stopwatch-284.glb', (gltf2) => {
          watchPct = 100; updatePct();
          pgWatchModel = gltf2.scene;
          pgWatchModel.traverse((n) => {
            if (!n.isMesh) return;
            n.material = new THREE.MeshStandardMaterial({ color: new THREE.Color(0xd0c8d8), metalness: 1.0, roughness: 0.04, envMapIntensity: 2.0 });
        ...
        }, (xhr2) => { if (xhr2.lengthComputable) { watchPct = (xhr2.loaded / xhr2.total) * 100; updatePct(); } }, () => { watchPct = 100; updatePct(); }
      );
    }, (xhr) => { if (xhr.lengthComputable) { enlargerPct = (xhr.loaded / xhr.total) * 100; updatePct(); } }, () => { pgModelFailed = true; enlargerPct = 100; watchPct = 100; updatePct(); canvas.classList.add('visible'); showBgFallback(); }
  );
}
...
function enlargerLoop() {
  ...
  if (pgWatchModel && productScroll) {
    const fraction = productScroll.scrollLeft / (productScroll.scrollWidth - productScroll.clientWidth || 1);
...
/* ═══════════════════════════════════════════════════════
   GLOBAL BACKGROUND 3D REEL PROJECTOR SHOWREEL ENGINE
════════════════════════════════════════════════════════ */
(function() {
