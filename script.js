...
/* ═══════════════════════════════════════════════════════
   ...
   Fixed positioning + restore if moshing was affected
════════════════════════════════════════════════════════ */

(function initMonitorPatch() {
  if (typeof THREE === 'undefined') return;
  
  // Wait for scroll-hint to exist
  let hints = document.querySelectorAll('.scroll-hint');
  if (hints.length === 0) {
    setTimeout(initMonitorPatch, 500);
    return;
  }
  ...
  const camera = new THREE.PerspectiveCamera(45, 280 / 380, 0.1, 1000);
  camera.position.z = 3;
  ...
  gltfLoader.load('portfolio/models/display_terminal.glb', (gltf) => {
    ...
    console.log('Monitor GLB failed to load');
    startRender();
  });
  ...
  function startRender() {
    ...
    function render() {
      ...
      if (monitorMesh) monitorMesh.rotation.y += 0.0008;
